...
public class ConcurrentLFU implements Cache {
	...
	public ConcurrentLFU(int sizeVal) {
	...
	public char get(int key) {
		...
		while (true) {
			...
			if (dataNode == null) {
				...
				synchronized (this) {
					boolean keyInserted = hashMap.containsKey(key);
					if (hashMap.size() >= maxSize && !keyInserted) {
						evict();
					}
					if (!keyInserted) {
